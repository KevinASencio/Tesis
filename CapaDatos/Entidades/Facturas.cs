...
namespace CapaDatos.Entidades
{
    public class Facturas
    {
        ...
        public DataTable ConsultarGenrar()
        {
            ...
            sentencia.Append("where fact.idservicio = serv.idservicio and cli.idcliente = serv.idcliente and ");
            sentencia.Append("serv.idconsumo = con.idserviciosconsumo and cuo.idcuotaconsumo = con.idcuotaconsumo and fact.estado='Valida' and fact.estado_pago='Pendiente' and ");
        ...
        public DataTable GenerarFacturas(int idcontrol, double mora)
        {
            ...
            foreach (DataRow rw in facturas.Rows)
            {
                ...
                {
                    sentencia.Append("insert into facturas (idfactura, saldo, mora, estado, estado_pago, idservicio, cont_pendiente, idcontrolfecha)");
                    sentencia.Append("values (" + 80664 + cont + ", ");
                    ...
                    try
                    {
                        ...
                        else Result.Rows.CopyTo(rw.ItemArray, cont);
                    }
                    catch (Exception ex)
                    {
